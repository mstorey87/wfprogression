...
There is then a separate function to create the false colour RGBs.
...
<!-- # Plot all images -->
<!-- plot all images at standard scale -->
<!-- ```{r} -->
...
# Line scans
Function to search line scan bounding box polygons charus database
Data are still being corrected and uploaded to the database, so this function does not work properly yet
...
# Progressions.
Search database on charus for fire progression polygons
There are no progressions for the example fire in the database yet, so as an example do a search with very broad dates to see if there are any progressions from other fires nearby
```{r}
...
Get the associated line scans names
Each polygon can have more than one scan in the s3name field, seperate by ;
The scans are not uploaded to storage grid yet, so we can download yet
```{r}
